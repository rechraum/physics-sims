const orbitalSketch = (p) => {
  ...
  p.setup = function () {
    ...
    clearButton = p.createButton('Clear Objects');
    clearButton.position(10, 10); // Position the button
    clearButton.mousePressed(clearObjects);

    // Create a 'Circular' checkbox
    circularCheckbox = p.createCheckbox('Circular', false);
    circularCheckbox.position(p.width / 2 - 100, 10); // Position in the upper right corner of XY plane
    circularCheckbox.style('color', 'black');       // Set text color

    // Adjust the checkbox and button styles
    circularCheckbox.style('background', 'white');
    clearButton.style('background', '#007acc');
  ...
  function drawPhaseSpaceAxes() {
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      let distanceValue = int(p.map(i, 0, 5, 0, p.width / 2));
      phaseSpace.noStroke();
