...
- ✅ `volatility-drag` — layout-c finance sim; contrasts arithmetic benchmark vs geometric expectation with one stochastic sample path; visualizes drag area and live σ²/2 readout; edu modes (Drag Intuition / Entropy Analogy / Applications) with canonical auto-sets for first two modes; preview: `node scripts/capture-previews.js volatility-drag`
